...
if brpylib_ver.split(".") < brpylib_ver_req.split("."):
    raise Exception(
        "requires brpylib " + brpylib_ver_req + " or higher, please use latest version"
    )
...
def plot_PSDs(znorm=False):
    for ii, plot_chan in enumerate(elec_ids):
        ...
        t = (
            cont_data["start_time_s"]
            + arange(cont_data["data"].shape[1]) / cont_data["samp_per_s"]
        )
        ...
        F, T, SG = sig.spectrogram(
            ...
            nperseg=256,
            noverlap=250,
            window=sig.get_window("blackmanharris", 256),
            fs=500,
        )
        plt.subplot(312)
...
def plot_TS(plotch=30, ds_f=40):
    ...
    tvect = np.linspace(
        cont_data["start_time_s"],
        cont_data["start_time_s"] + ts_sig.shape[0] / (fs / ds_f),
        ts_sig.shape[0],
    )
    plt.title(plotch)
...
def plot_SGs(plotch=30):
    ...
    F, T, SG = sig.spectrogram(
        ...
        nperseg=1024,
        noverlap=1000,
        window=sig.get_window("blackmanharris", 1024),
        fs=fs / ds_f,
        nfft=2 ** 11,
    )
    plt.title(plotch)
