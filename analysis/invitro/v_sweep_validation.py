...
stim_vs = {
    ...
    6: (160, 180),
    8: (220, 240),
}  # the timeperiods being studied as the 'voltage' times
do_stimvs = [0, 2, 4, 6, 8]
...
do_feats = {
    "Standard": ["Delta", "Theta", "Alpha", "Beta", "Gamma"],
    "Adjusted": ["Delta", "Theta", "Alpha", "Beta*", "Gamma1"],
}

for gel, fname in v_files.items():
    _ = spot_check(fname, tlims=(0, -1), plot_sg=True, plot_channs=[do_side])
    ...
    for stim_v, iv in stim_vs.items():
        ...
        state_vect, state_basis = dbo.calc_feats(
            ...
            modality="lfp",
            dofeats=do_feats[band_approach],
    ...
    for plot_proc in preproc_flows:
        ...
        for v_stim in do_stimvs:
            plt.subplot(2, 1, 1)
            plt.plot(
                exp_results[v_stim]["F"]["F"],
                10 * np.log10(exp_results[v_stim]["F"][plot_proc[0]][do_side]),
                linewidth=3,
            )
            plt.xlim((0, 70))
            ...
            plt.subplot(2, 2, 3)
            plt.plot(
                exp_results[v_stim]["Osc"]["Basis"],
                exp_results[v_stim][plot_proc[1]]["State"][:, side_idx[do_side]],
                linewidth=3,
            )
            ...
            plt.scatter(
                exp_results[v_stim]["THarm"][side_idx[do_side]],
                exp_results[v_stim]["SHarm"][side_idx[do_side]],
            )
            plt.ylabel("Secondary Harmonic")
